...
with uapi.ApiClient(configuration) as api_client:
    ...
    source = 'source_example' # str | 查询结果类型。不传时返回标准结果；如果设置为 `commercial`，将返回更完整的地理位置信息，但响应时间可能会稍长。 (optional)
 ...
 **source** | **str**| 查询结果类型。不传时返回标准结果；如果设置为 &#x60;commercial&#x60;，将返回更完整的地理位置信息，但响应时间可能会稍长。 | [optional] 
...
# **get_network_myip**
> GetNetworkMyip200Response get_network_myip(source=source)
