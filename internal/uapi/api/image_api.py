...
class ImageApi:
    ...
    def get_image_bing_daily(
        ...
        random: Annotated[Optional[StrictBool], Field(description="是否每次请求随机返回一张历史壁纸。传 `true` 时生效；不能和 `date` 同时使用。不传或传 `false` 时保持默认当天/指定日期逻辑。")] = None,
        ...
        :type var_date: str
        :param random: 是否每次请求随机返回一张历史壁纸。传 `true` 时生效；不能和 `date` 同时使用。不传或传 `false` 时保持默认当天/指定日期逻辑。
        :type random: bool
